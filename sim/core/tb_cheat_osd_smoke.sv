...
module tb_cheat_osd_smoke;
    ...
    reg reset=1, show=1, cart=0, ce=0;
    ...
    integer x, y, frame, ink_hdr=0, ink_rows=0, active_unused=0, xs=0, ink_right=0;
    task run_frame;
        begin
            ...
            for (y=0;y<V_ACTIVE;y=y+1) begin
                for (x=0;x<H_ACTIVE;x=x+1) begin
                    // Two clocks per pixel, as the adapter drives it; the
                    // pixel advances on the second (ce) and both must agree.
                    ...
                    if (frame==1) begin
                        ...
                        if (ink && x>=192) ink_right=ink_right+1;
                    end
                    ce=1; @(negedge clk);
                    if (frame==1 && (active===1'bx || ink===1'bx)) xs=xs+1;
                end
    ...
    initial begin
        ...
        if (ink_right!=0) $fatal(1,"FAIL header text repeated at the right edge: %0d ink pixels past x=192", ink_right);
